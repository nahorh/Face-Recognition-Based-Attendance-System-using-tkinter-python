...
def main_function():
    ...
    cam = VideoStream(src=0, resolution=(640, 480))
    ...
    def quitBtnFunction():
        proceed = messagebox.askyesno(
            'Close Application', 'Are you sure you want to exit the application?')
        if proceed:
            sys.exit()
        else:
            pass
    ...
    forgotPasswordBtnImg = PhotoImage(
        file='buttonImages/forgotpasswordButton.png')
        ...
        def TrainImages():
            try:
                ...
                faces, Id = getImagesAndLabels(
                    os.path.join(root_path, "TrainingImage"))
                print(faces)
                print(Id)
                recognizer.train(faces, np.array(Id))
                ...
                print(e)
                messagebox.showerror('Recognizer object creation error',
                                     'Please check that you have "opencv-contrib-python" installed on your system. If not then run this command in cmd "pip install opencv-contrib-python"')
            ...
            if(is_number(Id) and name.isalpha() and symbol in str(student_email) and symbol in str(parent_email)):
                ...
                while(True):
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        cv2.putText(img, 'Press c key to capture next image',
                                    (0, 40), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 0, 255), 1)
                    cv2.putText(img, 'Press q to close camera', (0, 20),
                    ...
                    if key == ord('c') or key == ord('C'):
                        ...
                        imgpath = os.path.join(
                            root_path, f'TrainingImage/{name}.{Id}.{sampleNum}.jpg')
                        ...
                        if sampleNum == 0:
                            cv2.destroyAllWindows()
                            messagebox.showinfo(
                                'No image captured', 'No image captured from camera. Signup discarded')
                            signUpWindow.deiconify()
                            return
                        break
                    if sampleNum > 9:
                ...
                if f'{Id},{name}' in data:
                    ...
                    messagebox.showwarning(
                        'User already present', f'User {name} already exists in the database. The captured images will be replaced with the previous ones')
                    TrainImages()
                    signUpWindow.attributes('-disabled', False)
                else:
                    ...
                    server.quit()
                    messagebox.showinfo(
                        'Email Sent', 'Your credentials have been sent to your email successfully. Kindly check that and login with the same next time')
                    signUpWindow.attributes('-disabled', False)
                ...
                tk.messagebox.showerror(
                    'Value Error', 'Please enter details in correct format')
            signUpWindow.attributes('-disabled', False)
    ...
    def forgotPasswd():
        ...
        def sendCreds():
            ...
            df = pd.DataFrame(df)
            try:
                extractedEmail = df.loc[df['Name'] ==
                                        uname]['StudentEmail'].values[0]
                extractedId = df.loc[df['Name'] == uname]['Id'].values[0]
            except:
                messagebox.showerror(
                    'Record Not Found', 'No record for this user exists in the system. Please enter correct details')
            ...
            if extractedEmail == uemail:
                ...
                messagebox.showerror(
                    'Incorrect email', 'Please enter the email correctly')
                forgotPasswordWindow.attributes('-disabled', False)
    ...
    # giving the heading
    message = Label(homeWindow, text="Face-Recognition-Based-Attendance-System",
                    bg="white", fg="black", width=100, height=2, font=('calibri', 30, 'bold'))
    message.pack(side='top', anchor='center')
    ...
    signInBtn = Button(topFrame, command=SignIn, image=signInBtnImg,
                       borderwidth=0, bg='white', activebackground='white')
    signInBtn.pack(side='top', pady=15)

    signUpBtn = Button(topFrame, command=SignUp, image=signUpBtnImg,
                       borderwidth=0, bg='white', activebackground='white')
    signUpBtn.pack(side='top', pady=15)

    forgotPasswordBtn = Button(topFrame, command=forgotPasswd, image=forgotPasswordBtnImg,
                               borderwidth=0, bg='white', activebackground='white')
    forgotPasswordBtn.pack(side='top', pady=15)

    quitBtn = Button(topFrame, command=quitBtnFunction, image=quitBtnImg,
                     borderwidth=0, bg='white', activebackground='white')
